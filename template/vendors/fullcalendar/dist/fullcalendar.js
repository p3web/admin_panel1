...
// Called on a entity to create a subclass.
...
// Adds new member variables/methods to the entity's prototype.
// Can be called with another entity, or a plain object hash containing new members.
...
function extendClass(superClass, members) {
	...
	// copy over all entity variables/methods to the subclass, such as `extend` and `mixin`
...
function Promise(executor) {
	...
	return promise; // instanceof Promise will break :( TODO: make Promise a real entity
}
...
// simple entity for toggle a `isIgnoringMouse` flag on delay
...
/* An abstract entity comprised of a "grid" of areas that each represent a specific datetime
----------------------------------------------------------------------------------------------------------------------*/
	...
	*/
	...
	fillSegHtml: function(type, seg) {
		...
		return '<' + this.fillSegTag +
			(classes.length ? ' entity="' + classes.join(' ') + '"' : '') +
			(css ? ' style="' + css + '"' : '') +
			' />';
	},
...
/* Event-rendering and event-interaction methods for the abstract Grid entity
----------------------------------------------------------------------------------------------------------------------*/
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadHtml: function() {
		...
		return '' +
			'<div entity="fc-row ' + view.widgetHeaderClass + '">' +
				'<table>' +
					'<thead>' +
	...
	renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
		...
		if (this.rowCnt === 1) {
			classNames = classNames.concat(
				// includes the day-of-week entity
				// noThemeHighlight=true (don't highlight the header)
				this.getDayClasses(date, true)
			);
		}
		else {
			classNames.push('fc-' + dayIDs[date.day()]); // only add the day-of-week entity
		}

		return '' +
            '<th entity="' + classNames.join(' ') + '"' +
				(this.rowCnt === 1 ?
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderDayRowHtml: function(row, isRigid) {
		...
		return '' +
			'<div entity="' + classes.join(' ') + '">' +
				'<div entity="fc-bg">' +
					'<table>' +
						this.renderBgTrHtml(row) +
					'</table>' +
				'</div>' +
				'<div entity="fc-content-skeleton">' +
					'<table>' +
						(this.numbersVisible ?
							'<thead>' +
	...
	renderNumberCellHtml: function(date) {
		...
		html += '<td entity="' + classes.join(' ') + '" data-date="' + date.format() + '">';
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			...
			var skeletonEl = $('<div entity="fc-helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
	...
	renderFillRow: function(type, seg, className) {
		...
		skeletonEl = $(
			'<div entity="fc-' + className + '-skeleton">' +
				'<table><tr/></table>' +
			'</div>'
		);
...
/* Event-rendering methods for the DayGrid entity
----------------------------------------------------------------------------------------------------------------------*/
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			...
			if (timeText) {
				timeHtml = '<span entity="fc-time">' + htmlEscape(timeText) + '</span>';
			}
		}

		titleHtml =
			'<span entity="fc-title">' +
				(htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
			'</span>';
		
		return '<a entity="' + classes.join(' ') + '"' +
				(event.url ?
			...
			'>' +
				...
				(isResizableFromStart ?
					'<div entity="fc-resizer fc-start-resizer" />' :
					''
					) +
				(isResizableFromEnd ?
					'<div entity="fc-resizer fc-end-resizer" />' :
					''
					) +
			'</a>';
	},
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $('<td entity="fc-event-container"/>').append(seg.el);
...
DayGrid.mixin({
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						moreTd = $('<td entity="fc-more-cell"/>').attr('rowspan', rowspan);
	...
	renderSegPopoverContent: function(row, col, segs) {
		...
		var content = $(
			'<div entity="fc-header ' + view.widgetHeaderClass + '">' +
				'<span entity="fc-close ' +
				...
				'<div entity="fc-clear"/>' +
			'</div>' +
			'<div entity="fc-body ' + view.widgetContentClass + '">' +
				'<div entity="fc-event-container"></div>' +
			'</div>'
		);
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				'<td entity="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
			...
			html +=
				'<tr data-time="' + slotDate.format('HH:mm:ss') + '"' +
					(isLabeled ? '' : ' entity="fc-minor"') +
					'>' +
					(!isRTL ? axisHtml : '') +
					'<td entity="' + view.widgetContentClass + '"/>' +
					(isRTL ? axisHtml : '') +
				"</tr>";
	...
	renderNowIndicator: function(date) {
		...
		for (i = 0; i < segs.length; i++) {
			nodes.push($('<div entity="fc-now-indicator fc-now-indicator-line"></div>')
				.css('top', top)
		...
		if (segs.length > 0) { // is the current time in view?
			nodes.push($('<div entity="fc-now-indicator fc-now-indicator-arrow"></div>')
				.css('top', top)
...
TimeGrid.mixin({
	...
	renderContentSkeleton: function() {
		...
		for (i = 0; i < this.colCnt; i++) {
			cellHtml +=
				'<td>' +
					'<div entity="fc-content-col">' +
						'<div entity="fc-event-container fc-helper-container"></div>' +
						'<div entity="fc-event-container"></div>' +
						'<div entity="fc-highlight-container"></div>' +
						'<div entity="fc-bgevent-container"></div>' +
						'<div entity="fc-business-container"></div>' +
					'</div>' +
				'</td>';
		}
...
/* An abstract entity from which other views inherit from
----------------------------------------------------------------------------------------------------------------------*/
...
var Scroller = FC.Scroller = Class.extend({
	...
	renderEl: function() {
		return (this.scrollEl = $('<div entity="fc-scroller"></div>'));
	},
...
function Toolbar(calendar, toolbarOptions) {
	...
	function render() {
		...
		if (sections) {
			if (!el) {
				el = this.el = $("<div entity='fc-toolbar "+ toolbarOptions.extraClasses + "'/>");
			}
			else {
				el.empty();
			}
	...
	function renderSection(position) {
		var sectionEl = $('<div entity="fc-' + position + '"/>');
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (overrideText) {
								...
								innerHtml = "<span entity='ui-icon ui-icon-" + themeIcon + "'></span>";
							}
							else if (normalIcon && !calendar.options.theme) {
								innerHtml = "<span entity='fc-icon fc-icon-" + normalIcon + "'></span>";
							}
							else {
							...
							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" entity="' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'
								)
								.click(function(ev) {
									...
									if (!button.hasClass(tm + '-state-disabled')) {
										...
										// it should never have a hover entity, so remove it now.
										if (
...
var Calendar = FC.Calendar = Class.extend({
	...
	// a lot of this entity' OOP logic is scoped within this constructor function,
	...
	buildViewSpec: function(requestedViewType) {
		...
		// iterate from the specific view definition to a more general one until we hit an actual View entity
		while (viewType) {
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		content = $("<div entity='fc-view-container'/>").prependTo(element);
	...
	function renderView(viewType, forcedScroll) {
		...
		if (!currentView && viewType) {
			...
			currentView.setElement(
				$("<div entity='fc-view fc-" + viewType + "-view' />").appendTo(content)
			);
...
/* An abstract entity for the "basic" views, as well as month view. Renders one or more rows of day cells.
----------------------------------------------------------------------------------------------------------------------*/
...
var BasicView = FC.BasicView = View.extend({
	...
	dayGridClass: DayGrid, // entity the dayGrid will be instantiated from (overridable by subclasses)
	...
	renderDates: function() {
		...
		var dayGridEl = $('<div entity="fc-day-grid" />').appendTo(dayGridContainerEl);
	...
	renderSkeletonHtml: function() {
		return '' +
			'<table>' +
				'<thead entity="fc-head">' +
					'<tr>' +
						'<td entity="fc-head-container ' + this.widgetHeaderClass + '"></td>' +
					'</tr>' +
				'</thead>' +
				'<tbody entity="fc-body">' +
					'<tr>' +
						'<td entity="' + this.widgetContentClass + '"></td>' +
					'</tr>' +
				'</tbody>' +
			'</table>';
	},
...
var basicDayGridMethods = {
	...
	renderHeadIntroHtml: function() {
		...
		if (view.colWeekNumbersVisible) {
			return '' +
				'<th entity="fc-week-number ' + view.widgetHeaderClass + '" ' + view.weekNumberStyleAttr() + '>' +
	...
	renderNumberIntroHtml: function(row) {
		...
		if (view.colWeekNumbersVisible) {
			return '' +
				'<td entity="fc-week-number" ' + view.weekNumberStyleAttr() + '>' +
	...
	renderBgIntroHtml: function() {
		...
		if (view.colWeekNumbersVisible) {
			return '<td entity="fc-week-number ' + view.widgetContentClass + '" ' +
	...
	renderIntroHtml: function() {
		...
		if (view.colWeekNumbersVisible) {
			return '<td entity="fc-week-number" ' + view.weekNumberStyleAttr() + '></td>';
		}
...
/* An abstract entity for all agenda-related views. Displays one more columns with time slots running vertically.
----------------------------------------------------------------------------------------------------------------------*/
...
var AgendaView = FC.AgendaView = View.extend({
	...
	timeGridClass: TimeGrid, // entity used to instantiate the timeGrid. subclasses can override
	timeGrid: null, // the main time-grid subcomponent of this view

	dayGridClass: DayGrid, // entity used to instantiate the dayGrid. subclasses can override
	...
	renderDates: function() {
		...
		var timeGridEl = $('<div entity="fc-time-grid" />').appendTo(timeGridWrapEl);
		...
		this.bottomRuleEl = $('<hr entity="fc-divider ' + this.widgetHeaderClass + '"/>')
	...
	renderSkeletonHtml: function() {
		return '' +
			'<table>' +
				...
				'<tbody entity="fc-body">' +
					'<tr>' +
						'<td entity="' + this.widgetContentClass + '">' +
							(this.dayGrid ?
								'<div entity="fc-day-grid"/>' +
								'<hr entity="fc-divider ' + this.widgetHeaderClass + '"/>' :
								''
								) +
						'</td>' +
					'</tr>' +
...
var agendaTimeGridMethods = {
	...
	renderHeadIntroHtml: function() {
		...
		if (view.opt('weekNumbers')) {
			...
			return '' +
				'<th entity="fc-axis fc-week-number ' + view.widgetHeaderClass + '" ' + view.axisStyleAttr() + '>' +
					view.buildGotoAnchorHtml( // aside from link, important for matchCellWidths
			...
			return '<th entity="fc-axis ' + view.widgetHeaderClass + '" ' + view.axisStyleAttr() + '></th>';
		}
	},
	...
	renderBgIntroHtml: function() {
		...
		return '<td entity="fc-axis ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '></td>';
	},
	...
	renderIntroHtml: function() {
		...
		return '<td entity="fc-axis" ' + view.axisStyleAttr() + '></td>';
	}
...
var agendaDayGridMethods = {
	...
	renderBgIntroHtml: function() {
		...
		return '' +
			'<td entity="fc-axis ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
				'<span>' + // needed for matchCellWidths
					view.getAllDayHtml() +
				'</span>' +
			'</td>';
...
var ListViewGrid = Grid.extend({
	...
	renderEmptyMessage: function() {
		this.el.html(
			'<div entity="fc-list-empty-wrap2">' + // TODO: try less wraps
			'<div entity="fc-list-empty-wrap1">' +
			'<div entity="fc-list-empty">' +
	...
	renderSegList: function(allSegs) {
		...
		var daySegs;
		var i;
		var tableEl = $('<table entity="fc-list-table"><tbody/></table>');
	...
	dayHeaderHtml: function(dayDate) {
		...
		return '<tr entity="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' +
			'<td entity="' + view.widgetHeaderClass + '" colspan="3">' +
				(mainFormat ?
					view.buildGotoAnchorHtml(
						dayDate,
	...
	fgSegHtml: function(seg) {
		...
		return '<tr entity="' + classes.join(' ') + '">' +
			(this.displayEventTime ?
				'<td entity="fc-list-item-time ' + view.widgetContentClass + '">' +
					(timeHtml || '') +
				'</td>' :
				'') +
			'<td entity="fc-list-item-marker ' + view.widgetContentClass + '">' +
				'<span entity="fc-event-dot"' +
				(bgColor ?
					' style="background-color:' + bgColor + '"' :
					'') +
				'></span>' +
			'</td>' +
			'<td entity="fc-list-item-title ' + view.widgetContentClass + '">' +
